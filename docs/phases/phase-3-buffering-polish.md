...
- [ ] **에러 핸들링**
  - [ ] 자막 없는 영상 안내
    - timedtext API 응답 empty → 사용자 메시지 표시
    - 메시지: "이 영상에는 자막이 없습니다. 다른 영상을 시도해주세요."
  - [ ] Claude 구독 한도 초과 안내
    - subprocess stderr에서 `rate limit` 또는 `exceeded` 감지
    - 메시지: "Claude Code 구독 한도를 초과했습니다. 나중에 다시 시도해주세요."
  - [ ] Claude CLI 미설치 안내
    - 앱 시작 시 `claude --version` 실행 실패
    - 메시지: "Claude Code CLI가 설치되지 않았습니다. `npm install -g @anthropic-ai/claude-code` 실행 후 다시 시도해주세요."
  - [ ] 번역 중 프로세스 비정상 종료 복구
    - SIGTERM/SIGKILL 감지 → 해당 청크 상태를 pending으로 변경
    - 사용자 메시지: "번역 중 오류가 발생했습니다. 자막을 다시 로드합니다."
    - 자동 재시도: 최대 3회
...
export function SubtitleDisplay({ 
  ...
  useEffect(() => {
    ...
    // 2. 로딩 상태 확인
    if (loadingState.seeking && !current) {
      setDisplayText('번역 준비 중...');
    } else if (current) {
      setDisplayText(current.translated);
    } else {
      setDisplayText('');
    }
  }, [currentTime, subtitles, loadingState]);

  return (
    <div className="subtitle-container">
      <p className={loadingState.seeking ? 'loading' : ''}>
        {displayText}
      </p>
    </div>
  );
...
**Phase 3 완료 조건**: 전체 15개 Acceptance Criteria 모두 PASS + 성능 벤치마크 달성 + 긴 영상 안정성 검증
