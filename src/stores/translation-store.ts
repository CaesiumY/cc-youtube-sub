...
export const useTranslationStore = create<TranslationState>((set, get) => ({
  ...
  //
  // `isLoading`("초기 로딩" — 자막 fetch ~ 첫 청크가 끝날 때까지)은 여기서 **끄기만** 한다.
  // 다시 켜지는 않는다 — BufferManager는 재생 위치 기준 LOOK_AHEAD 청크만 lazy 번역하므로
  // "모든 청크 완료" 같은 전역 조건으로 로딩을 판정할 수 없다. 재생 중 "아직 번역 안 된
  // 구간" 판정은 subtitle-overlay가 현재 재생 위치의 청크 status로 파생 계산한다.
  //
  // 첫 청크가 어떤 종료 상태(done/cached/error)에든 도달하면 초기 로딩 단계는 끝났다.
  // error도 포함하는 게 중요하다 — 첫 청크가 실패해도 '번역 준비 중' 표시를 풀어야 한다
  // (안 그러면 첫 청크 실패 시 오버레이가 초기 로딩에 영원히 멈춘다).
  markChunkStatus: (index, status) => {
    const { chunkStatuses, completedChunks, cachedChunks, isLoading } = get();
    const prev = chunkStatuses[index];
    const wasDone = prev === "done" || prev === "cached";
    const isDone = status === "done" || status === "cached";
    const isTerminal = isDone || status === "error";
    ...
    set({
      ...
      ...(isLoading && isTerminal ? { isLoading: false } : {}),
    });
  },
